...
async function mainProcess() {
  await loadBookingIntervalsConfig();
...
async function loadBookingIntervalsConfig() {
  try {
    const response = await fetch("./booking-intervals.json", { cache: "no-store" });
    if (!response.ok) {
      throw new Error(`Unable to load booking-intervals.json (HTTP ${response.status})`);
    }
    bookingIntervalsConfig = await response.json();
  } catch (error) {
    console.error("Failed to load booking intervals config:", error);
    bookingIntervalsConfig = {};
  }
}
...
// Intervalli delle prenotazioni letti da file JSON per giorno e biblioteca
function getBookingIntervals(libName, dayName){
  if(dayName === "Saturday" || dayName === "Sunday") return [];

  return bookingIntervalsConfig?.[libName]?.[dayName] || [];
}
